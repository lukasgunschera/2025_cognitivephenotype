## ======================================================================================================================= ##
## Script:    FUNCTION: MODEL COMPARISON
## ======================================================================================================================= ##
## Authors:   Lukas J. Gunschera
## Date:      Tue Apr 23 17:31:44 2024
## ======================================================================================================================= ##
...
model_comparison <- function(model_paths = NULL,
  ...
  if (LOO_plot) {
    ...
    model_comp_loo_plot <- ggplot(model_comp_loo_dat, aes(x = model, y = loo)) +
      ...
      geom_errorbar(aes(ymin = loo - loo_sd, ymax = loo + loo_sd),
        width = .2,
  ...
  if (ELPD_plot) {
    ...
    model_comp_elpd_plot <- ggplot(model_comp_elpd_dat, aes(x = model, y = elpd)) +
      ...
      geom_errorbar(aes(ymin = elpd - elpd_sd, ymax = elpd + elpd_sd),
        width = .2,
        position = position_dodge(.9)
      ) +
      # geom_text(aes(label = round(elpd)), position = position_dodge(0.9), hjust=-0.5, size = 3) +
      ylab("Expected log posterior density (± SE)") +
      xlab("Model") +
      ggtitle(ELPD_title) +
      coord_flip() +
      theme(
  ...
  if (LOO_ELPD_plot) {
    ...
    model_comp_loo_elpd_plot <- ggplot(model_comp_loo_elpd_dat, aes(x = model, y = loo_elpd, fill = measure)) +
      ...
      geom_errorbar(aes(ymin = loo_elpd - loo_elpd_sd, ymax = loo_elpd + loo_elpd_sd),
        width = .2,
        position = "identity"
      ) +
      ylab("Information criterion (± SE)") +
      xlab("") +
