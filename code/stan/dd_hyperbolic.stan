...
data {
  ...
  array[N, T] real<lower=0> delay_sooner; // effort of offer
  array[N, T] real<lower=0> amount_sooner; // reward of offer
  array[N, T] real<lower=0> delay_later; // effort of reject option
  array[N, T] real<lower=0> amount_later; // reward of reject option
  array[N, T] int<lower=-1, upper=1> choice; // 0 for rejected offer, 1 for accepted delayed offer
}
...
generated quantities {
  ...
  // Set all posterior predictions (avoids NULL values)
  for (i in 1:N) {
    for (t in 1:T) {
      y_pred[i, t] = -1;
    }
